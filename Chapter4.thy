...
fun ord :: "int tree \<Rightarrow> bool"  where
    "ord Tip = False" |
    "ord (Node l x r) = 
    ((\<forall> y \<in> set l. x \<ge> y) \<and> (\<forall> y \<in> set r. y \<ge> x) \<and> ord l \<and> ord r)"
...
fun ins :: "int \<Rightarrow> int tree \<Rightarrow> int tree"  where
    ...
    "ins v (Node l x r) = 
        (if ((\<exists> i \<in> set l . v = i) \<or> (\<exists> i \<in> set r . v = i) \<or> (x = v))
            then (Node l x r)
            else (if (x < v)
                    then (Node l x (ins v r))
                    else (Node (ins v l) x r)))"
